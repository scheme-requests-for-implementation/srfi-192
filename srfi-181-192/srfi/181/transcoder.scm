...
(define (make-u8-ascii-read! source handling)
  (define (read-1)
...
(define (make-u16-ascii-read! source handling)
  (define endianness #f)
  (define (read-1)
  ...
  (define (decode-1 hi lo)
    (cond ((and (zero? hi) (< lo #x80)) (integer->char lo))
          ((eq? handling 'replace) #\?)
...
(define (make-u8-ascii-write! sink handling)
  (define (write-1 c)
    (write-u8 (char->integer c) sink))
  (make-flusher write-1))

(define (make-u16-ascii-write! sink handling)
  ;; we assume BE
    ...
    (case (codec-name (transcoder-codec transcoder))
      ((latin-1 utf-8)
       (make-custom-textual-input-port (port-id)
                                       (make-u8-ascii-read! inner (transcoder-handling-mode transcoder))
                                       #f #f closer))
      ((utf-16)
       (make-custom-textual-input-port (port-id)
                                       (make-u8-ascii-read! inner (transcoder-handling-mode transcoder))
    ...
    (case (codec-name (transcoder-codec transcoder))
      ((latin-1 utf-8)
       (make-custom-textual-output-port (port-id)
                                        (make-u8-ascii-write! inner (transcoder-handling-mode transcoder))
                                        #f #f closer))
      ((utf-16)
       (make-custom-textual-output-port (port-id)
                                        (make-u16-ascii-write! inner (transcoder-handling-mode transcoder))
                                        #f #f closer))))
   (else
